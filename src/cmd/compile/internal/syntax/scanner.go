...
func (this *scanner) setNoAutoSemi(){
   this.mode = this.mode | noAutoSemi;
   this.nlsemi = false;
   return;
   }
   
func (this *scanner) setAutoSemi(){
   this.mode = this.mode &^ noAutoSemi;
   this.nlsemi = true;
   return;
   }
...
func (s *scanner) lineComment() {
	...
	// are we saving directives? or is this definitely not a directive?
      if s.mode&directives == 0 || (s.ch != 'g' && s.ch != 'l' && s.ch != 'A') {
         s.stop()
         s.skipLine()
         return
         }
...
func (s *scanner) fullComment() {
	/* opening has already been consumed */
      ...
      if s.mode&directives == 0 || (s.ch != 'l' && s.ch != 'A') {
         s.stop()
         s.skipComment()
         return
         }
