...
// goroot/src/cmd/compile/internal/syntax/
...
func (p *parser) init(file *PosBase, r io.Reader, errh ErrorHandler, pragh PragmaHandler, mode Mode) {
	...
	p.pragh = pragh
	scanMode := directives;
	   if mode & NoAutoSemi != 0{
	   	  scanMode = scanMode | noAutoSemi;
	      }
	directiveHandler := func(line uint, col uint, msg string){p.processDirective(line,col,msg);};
	p.scanner.init(r,directiveHandler,scanMode,)
...
func (this *parser) processDirective(line uint,col uint,msg string){
	
      if msg[0] == '/' {
         text := commentText(msg);
            if strings.HasPrefix(text,"line ") {
            	this.lineDirective(line,col,msg,text);
            } else if strings.HasPrefix(text,"AutoSemi") {
            	this.autoSemiDirective(line,col,msg,text);
            } else if strings.HasPrefix(text,"go:") {
               this.goDirective(line,col,text);
               }
      } else {
         this.errorAt(this.posAt(line, col), msg); 
         }
   return;
   }

func (this *parser) lineDirective(line uint,col uint,msg string,text string) {
   var pos Pos;
   posUpdate := false;
      if(col == colbase && msg[1] == '/'){
      	 pos = MakePos(this.file,line+1,colbase);
      	 posUpdate = true;
      } else if msg[1] == '*' {
      	 pos = MakePos(this.file,line,col + uint(len(msg)));
      	 posUpdate = true;
         }
      if posUpdate == true {
         this.updateBase(pos,line,col + 7, text[5:]);
         }
   return;
   }
...
func (this *parser) autoSemiDirective(line uint,col uint,msg string,text string) {
	...
	dlen = 0; 
      ...
      if dlen > 0 { 
         ...
         this.updateBase(pos,line,col + dlen + 2, text[dlen:]);
      }
   return;
   }
   
func (this *parser) goDirective(line uint,col uint,text string) {
      if this.pragh != nil {
         this.pragma = this.pragh(this.posAt(line, col+2), this.scanner.blank, text, this.pragma);
         }
   return;
   }
...
func (p *parser) stmtList() (l []Stmt) {
      ...
      for p.tok != _EOF && p.tok != _Rbrace && p.tok != _Case && p.tok != _Default {
		...
		// ";" is optional before "}"
            ...
            if !p.got(_Semi) && p.tok != _Rbrace && p.mode & NoAutoSemi == 0 && p.previousMode == p.mode{
